...
int main(int argc, char* argv[]) {
	...
	if(!strcmp(outFile, "-")) ofd = fileno(stdout); // Case: user wants to write to stdout
		...
		ofd = open(argv[2], O_WRONLY | O_CREAT, 00200);	// If no file exists, create one with write-only permission
	...
	/////////////////////////////////////
	// Read input and perform counting //
	/////////////////////////////////////
	...
	if(ofd==fileno(stdout)) {	/* Case: writing to stdout */	
		...
		write(ofd, catsToString(ccs1), getArrayLength(catsToString(ccs1)));
