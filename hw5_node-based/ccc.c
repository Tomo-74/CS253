...
extern void addCat(char* name, char* targetChars) {
	Node* newNode = (Node *)malloc(sizeof(Node));	// Allocate space for a new node

	// Link the new node to the list
	if(numCats == 0) {		// Case: first node is added
		head = newNode;
		tail = newNode;
		tail->nextNode = NULL;
	} else {			// Case: all subsequent additions
		tail->nextNode = newNode;
	}

	// Initialize the new category stored in the node
	newNode->category->name = name;
	newNode->category->targetChars = targetChars;
	newNode->category->count = 0;
...
extern void ccc(char* input, ssize_t inputLen) {	
	...
	for(Node* tmp=head; tmp!=NULL; tmp=tmp->nextNode) {
		int numTargetChars = getArrayLength(tmp->category->targetChars);
		tmp->category->count = countOccurences(input, inputLen, tmp->category->targetChars, numTargetChars);	// Count the number of char occurences for that category and update the count variable		
		printf("%s %d\n", tmp->category->name, tmp->category->count);	// Print results to console
	}
